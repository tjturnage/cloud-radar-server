"""
This script takes a boilerplate html file called PAGE and replaces the [address] placeholder
with the unique number string associated with the particular simulation being run to provide
the unique links for the simulation to the user.
"""
import sys
...
<br><hr><br>
    <div class="container">
        ...
        <div class="d-flex flex-wrap desc">Surface-1-km Vertical Shear is the difference between the surface wind and the wind at 1-km 
            above ground level. These data are plotted as vectors. 0-1-km shear magnitudes greater than 15-20 knots tend to favor 
            supercell tornadoes.<div>
        <br>
        <div class="d-flex flex-wrap title">0-3km Shear</div>
        <div class="d-flex flex-wrap desc">
            Surface-3-km Vertical Shear is the difference between the surface wind and the wind 
            at 3-km above ground level. Line-normal 0-3 km shear magnitudes of 30 kt or higher indicate a favorable environment 
            for the development of mesovortices and/or tornadoes in QLCSs (Schaumann and Przybylinski 2012). These data are plotted 
            as vectors.
        </div>
        <br>
        ...
        <div class="d-flex flex-wrap desc">
            ...
            Thunderstorms tend to become more organized and persistent as vertical shear increases. Supercells are commonly 
            associated with vertical shear values of 35-40 knots and greater through this depth.
        </div>
        <br>
        ...
        <div class="d-flex flex-wrap desc">
            The Significant Tornado Parameter (effective layer) is a multiple ingredient, composite index that 
            includes effective bulk wind difference (EBWD), effective storm-relative helicity (ESRH), 100-mb mean 
            parcel CAPE (mlCAPE), 100-mb mean parcel CIN (mlCIN), and 100-mb mean parcel LCL height (mlLCL). The 
        ...
        <br>
        <div class="d-flex flex-wrap desc">
            This normalized parameter is meant to highlight areas where steep low-level lapse rates correspond with low-level 
            instability, little convective inhibition, weak deep-layer vertical shear, and large cyclonic surface vorticity. 
            Values > 1 suggest an enhanced potential for non-mesocyclone tornadoes.
            <br><br>
            <i>Note that this placefile uses the same Baumgardt and Cook relative vorticity divisor (in term 5) as the SPC 
            mesoanalysis. However, these placefiles run on a 13km grid (vs. 40km for SPC) and thus the magnitude of NST output 
            may be greater than what appears on the SPC page.</i>
        </div>
        <br>
...
class WriteLinksPage():
    ...
    def create_links_page(self) -> None:
        """
        writes the unique links page for the simulation
        """
